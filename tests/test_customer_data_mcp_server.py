"""Tests for the customer_data MCP server (mcp_servers/customer_data/main.py).

Covers:
    * Service PASETO validation (rejects unsigned/expired/wrong-keypair).
    * Contract: tool names + input schemas match what the orchestrator
      SKILL.md will declare for US-010.
    * Tool dispatch: ``get_customer`` / ``list_accounts`` / ``get_device_history``
      each hit the customer_data mock API and return MCP-shaped results
      (text + structured data).
    * End-to-end integration: MCP gateway -> customer_data MCP server ->
      customer_data mock API yields the expected payload.
...
def mock_api_client() -> Iterator[httpx.AsyncClient]:
    """In-process ASGI client into the customer_data mock API (US-008)."""
    mock_app = create_mock_app()
    transport = httpx.ASGITransport(app=mock_app)
    client = httpx.AsyncClient(transport=transport, base_url="http://mock")
    yield client
    # TestClient lifecycles run inside the same loop; let it close on its own.
...
def _service_token(
    *, service_priv: Path, sub: str = "alice@example.com", ttl: int = 60
    ...
    claims = Claims(
        ...
        allowed_tools={"customer_data": ["get_customer", "list_accounts", "get_device_history"]},
        trace_id="trace-abc",
    )
...
def test_tool_names_constant_matches_prd() -> None:
    """PRD US-009: server must expose get_customer, list_accounts, get_device_history."""
    assert set(TOOL_NAMES) == {"get_customer", "list_accounts", "get_device_history"}


def test_server_name_matches_gateway_url_segment() -> None:
    """Server name MUST equal the gateway's f'{downstream_url}/{server}' suffix."""
    assert SERVER_NAME == "customer_data"
...
def test_build_mcp_registers_three_tools() -> None:
    ...
    try:
        ...
        assert names == {"get_customer", "list_accounts", "get_device_history"}
    finally:
...
def test_missing_authorization_is_rejected(
    ...
    _, service_pub = service_keys
    client = _server_client(service_pub=service_pub, mock_client=mock_api_client)
    resp = client.post(f"/{SERVER_NAME}", json=_rpc_call("get_customer", {"customer_id": "c-1"}))
...
def test_wrong_keypair_token_is_rejected(
    ...
    _, service_pub = service_keys
    # Mint with a wholly unrelated keypair.
...
def test_tools_list_schemas_match_orchestrator_skill_contract(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    """Contract test for US-010: the orchestrator SKILL.md will declare these tools.

    Any SKILL.md change that adds/renames a tool or changes a required param
    must be reflected here AND in the SKILL.md frontmatter. Two-way fence.
    """
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    client = _server_client(service_pub=service_pub, mock_client=mock_api_client)
    ...
    # The orchestrator SKILL.md contract for US-010 (gather-customer-profile).
    expected_contract = {
        "get_customer": {"required": ["customer_id"], "optional": ["scenario"]},
        "list_accounts": {"required": ["customer_id"], "optional": ["scenario"]},
        "get_device_history": {"required": ["customer_id"], "optional": ["scenario"]},
    }
...
def _assert_mcp_shape(body: dict[str, Any]) -> dict[str, Any]:
    """Common assertions: JSON-RPC result wraps {content: [...], structuredContent: {...}}."""
    assert body["jsonrpc"] == "2.0"
...
def test_get_customer_returns_profile(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("get_customer", {"customer_id": "c-42", "scenario": "clean"}),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200, resp.text
    structured = _assert_mcp_shape(resp.json())
    assert structured["customer_id"] == "c-42"
    assert structured["scenario"] == "clean"
    assert "risk_score" in structured
    assert "flags" in structured
...
def test_list_accounts_returns_account_list(
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("list_accounts", {"customer_id": "c-7", "scenario": "mule"}),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200, resp.text
    structured = _assert_mcp_shape(resp.json())
    assert structured["customer_id"] == "c-7"
    assert structured["scenario"] == "mule"
    assert isinstance(structured["accounts"], list)
    assert len(structured["accounts"]) >= 3  # mule scenario has 3+


def test_get_device_history_returns_device_list(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("get_device_history", {"customer_id": "c-9", "scenario": "ato"}),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200, resp.text
    structured = _assert_mcp_shape(resp.json())
    assert structured["customer_id"] == "c-9"
    assert structured["scenario"] == "ato"
    assert isinstance(structured["devices"], list)
    assert any(d.get("suspicious") for d in structured["devices"])


def test_scenario_omitted_uses_mock_default(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    """When the caller omits ?scenario, the mock picks a deterministic default."""
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    client = _server_client(service_pub=service_pub, mock_client=mock_api_client)
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("get_customer", {"customer_id": "c-default-test"}),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200, resp.text
    structured = _assert_mcp_shape(resp.json())
    assert structured["customer_id"] == "c-default-test"
...
def test_invalid_arguments_for_tool_returns_400(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    """Missing required ``customer_id`` should propagate as a tool error."""
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    client = _server_client(service_pub=service_pub, mock_client=mock_api_client)
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("get_customer", {}),  # no customer_id
        headers={"Authorization": f"Bearer {token}"},
    )
    # FastMCP runs pydantic-style validation; failures bubble up. We don't pin
    # the exact code — just that it's an error, not a success.
    body = resp.json()
    assert resp.status_code >= 400 or "error" in body
...
def test_unknown_scenario_surfaces_as_upstream_error(
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call(
            "get_customer", {"customer_id": "c-1", "scenario": "nonsense"}
        ),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 400
...
# --------------------------------------------------------------------------- #
# End-to-end: MCP gateway -> customer_data server -> customer_data mock       #
# --------------------------------------------------------------------------- #
...
def test_gateway_to_server_to_mock_end_to_end(
    ...
    """Integration: a user PASETO into the MCP gateway returns the mock payload.

    The gateway re-signs a service PASETO that the customer_data MCP server
    accepts; the server then calls the mock API and returns the MCP-shaped
    result back through the gateway to the caller.
    """
    inbound_priv, inbound_pub = inbound_keys
    service_priv, service_pub = service_keys

    # Build the downstream server (validates service PASETO with service_pub)
    # using the in-process mock API client.
    server_app = create_app(
        public_key_path=service_pub,
        api_client=mock_api_client,
    )
    server_transport = httpx.ASGITransport(app=server_app)
    # The gateway POSTs to f"{downstream_url}/{server}" — give it a base URL
    # whose ASGI transport routes to the server app's f"/{SERVER_NAME}" route.
    ...
    # Mint a user token the gateway accepts.
    user_claims = Claims(
    ...
    resp = gateway_client.post(
        f"/mcp/{SERVER_NAME}",
        json=_rpc_call(
            "get_customer", {"customer_id": "c-e2e", "scenario": "sanctions_hit"}
        ),
    ...
    assert structured["pep"] is True
    assert structured["risk_score"] >= 80
    assert "sanctions_watchlist_possible" in structured["flags"]

    # Audit trail recorded exactly one row.
    ...
    assert row["trace_id"] == "trace-e2e"


def test_end_to_end_list_accounts_for_mule(
    inbound_keys: tuple[Path, Path],
    service_keys: tuple[Path, Path],
    mock_api_client: httpx.AsyncClient,
    memory_audit_backend: SQLiteAuditBackend,
) -> None:
    """list_accounts path through the gateway for the mule scenario."""
    inbound_priv, inbound_pub = inbound_keys
    service_priv, service_pub = service_keys
    ...
    user_claims = Claims(
        ...
        roles=["analyst"],
        allowed_servers=["customer_data"],
        allowed_tools={"customer_data": ["list_accounts"]},
        trace_id="trace-mule",
    )
    user_token = mint(user_claims, ttl_seconds=300, private_key_path=inbound_priv)
    ...
    resp = gateway_client.post(
        f"/mcp/{SERVER_NAME}",
        json=_rpc_call(
            "list_accounts", {"customer_id": "c-mule", "scenario": "mule"}
        ),
        headers={"Authorization": f"Bearer {user_token}"},
    )
    assert resp.status_code == 200, resp.text
    structured = _assert_mcp_shape(resp.json())
    assert len(structured["accounts"]) >= 3
    assert structured["scenario"] == "mule"
...
def test_end_to_end_determinism(
    ...
    def _call() -> dict[str, Any]:
        ...
        resp = gateway_client.post(
            f"/mcp/{SERVER_NAME}",
            json=_rpc_call("get_customer", {"customer_id": "c-d1", "scenario": "clean"}),
            headers={"Authorization": f"Bearer {token}"},
        )
        assert resp.status_code == 200
