"""Tests for the sanctions MCP server (mcp_servers/sanctions/main.py).

Covers:
    * Service PASETO validation (rejects unsigned/expired/wrong-keypair).
    * Contract: tool names + input schemas (US-014 acceptance).
    * Tool dispatch: ``screen_name`` / ``screen_entity`` / ``get_watchlist_hit``
      each hit the sanctions mock API and return MCP-shaped results.
    * End-to-end: MCP gateway -> sanctions MCP server -> sanctions mock API
...
from mcp_servers.sanctions.main import (
    DEFAULT_API_URL,
    SERVER_NAME,
    TOOL_NAMES,
    build_mcp,
...
from mock_apis.sanctions.main import create_app as create_mock_app
...
def mock_api_client() -> Iterator[httpx.AsyncClient]:
    """In-process ASGI client into the sanctions mock API (US-014)."""
...
def test_tool_names_constant_matches_prd() -> None:
    """PRD US-014: server must expose three tools."""
...
def test_missing_authorization_is_rejected(
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("screen_name", {"name": "Alice Smith"}),
    )
...
def test_tools_list_schemas_match_sanctions_contract(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    """Contract test: the sanctions tools must declare these signatures.

    Two-way fence with the screen-sanctions SKILL.md (US-019). If a tool
    parameter set changes, this test changes in lockstep with the SKILL.md
    frontmatter that references the sanctions server.
    """
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    client = _server_client(service_pub=service_pub, mock_client=mock_api_client)
...
def test_screen_name_clean_scenario(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("screen_name", {"name": "Alice Smith", "scenario": "clean"}),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200, resp.text
    structured = _assert_mcp_shape(resp.json())
    assert structured["matched"] is False
    assert structured["hits"] == []
    assert structured["query"] == "Alice Smith"


def test_screen_name_sanctions_hit_returns_match(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call(
            "screen_name", {"name": "Alice Smith", "scenario": "sanctions_hit"}
        ),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200, resp.text
    ...
    assert len(structured["hits"]) >= 1
    hit = structured["hits"][0]
    assert hit["entity_type"] == "natural_person"


def test_screen_entity_sanctions_hit_returns_entity_match(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    ...
    resp = client.post(
        ...
        json=_rpc_call(
            "screen_entity",
            {"entity_name": "Shell Holdings Ltd", "scenario": "sanctions_hit"},
        ),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200, resp.text
...
def test_get_watchlist_hit_returns_detail(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    client = _server_client(service_pub=service_pub, mock_client=mock_api_client)
    # First screen to get a valid hit_id.
    ...
    hit_id = _assert_mcp_shape(screen_resp.json())["hits"][0]["hit_id"]

    detail_resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("get_watchlist_hit", {"hit_id": hit_id}),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert detail_resp.status_code == 200, detail_resp.text
    detail = _assert_mcp_shape(detail_resp.json())
    assert detail["hit_id"] == hit_id
    assert "program" in detail
    assert "aliases" in detail


def test_get_watchlist_hit_unknown_id_surfaces_404(
    service_keys: tuple[Path, Path], mock_api_client: httpx.AsyncClient
) -> None:
    service_priv, service_pub = service_keys
    token = _service_token(service_priv=service_priv)
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("get_watchlist_hit", {"hit_id": "hit_does_not_exist"}),
        headers={"Authorization": f"Bearer {token}"},
    )
...
def test_unknown_tool_returns_method_not_found(
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call("nope", {"name": "Alice"}),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 404
...
def test_unknown_scenario_surfaces_as_upstream_error(
    ...
    resp = client.post(
        f"/{SERVER_NAME}",
        json=_rpc_call(
            "screen_name", {"name": "Alice", "scenario": "nonsense"}
        ),
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 400
...
# --------------------------------------------------------------------------- #
# End-to-end: MCP gateway -> sanctions server -> sanctions mock               #
# --------------------------------------------------------------------------- #
...
def test_gateway_to_server_to_mock_end_to_end(
    ...
    """User PASETO -> gateway -> sanctions server -> mock returns expected payload."""
    ...
    resp = gateway_client.post(
        ...
        json=_rpc_call(
            "screen_name",
            {"name": "Alice Smith", "scenario": "sanctions_hit"},
        ),
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row["server"] == "sanctions"
    assert row["tool"] == "screen_name"
    assert row["status"] == "ok"
    assert row["trace_id"] == "trace-e2e-sanctions"


def test_gateway_to_server_to_mock_get_hit_e2e(
    inbound_keys: tuple[Path, Path],
    service_keys: tuple[Path, Path],
    mock_api_client: httpx.AsyncClient,
    memory_audit_backend: SQLiteAuditBackend,
) -> None:
    """Two-hop e2e: first screen, then resolve hit detail through the gateway."""
    inbound_priv, inbound_pub = inbound_keys
    service_priv, service_pub = service_keys
    ...
    user_claims = Claims(
        ...
        trace_id="trace-e2e-sanctions-2",
    )
    # Mint a fresh user token per call — the MCP gateway tracks jti to
    # prevent replay, so the same token can't make two calls.
    screen_resp = gateway_client.post(
        f"/mcp/{SERVER_NAME}",
        json=_rpc_call(
            "screen_name",
            {"name": "Alice Smith", "scenario": "sanctions_hit"},
        ...
        headers={
            "Authorization": (
                f"Bearer {mint(user_claims, ttl_seconds=300, private_key_path=inbound_priv)}"
            )
        },
    )
    hit_id = _assert_mcp_shape(screen_resp.json())["hits"][0]["hit_id"]

    detail_resp = gateway_client.post(
        f"/mcp/{SERVER_NAME}",
        json=_rpc_call("get_watchlist_hit", {"hit_id": hit_id}),
        headers={
            "Authorization": (
    ...
    memory_audit_backend.flush()
    rows = memory_audit_backend.query(sub="alice@example.com")
    # screen_name + get_watchlist_hit = 2 audit rows
    assert len(rows) == 2
    tools = {r["tool"] for r in rows}
    assert tools == {"screen_name", "get_watchlist_hit"}
